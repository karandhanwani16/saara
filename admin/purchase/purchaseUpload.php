...
function loadSuppliers($con)
{
    $options = "";
    $query = "select * from supplier";
    $result = $con->query($query);
    while ($row = $result->fetch_assoc()) {
        $options .= "<option value='" . $row['supplier_id'] . "' >" . $row['supplier_name'] . "</option>";
    }
    return $options;
}
    ...
    <form action="#">
        <div class="inp-row row-3">
            ...
            <div class="inp-group">
                ...
                <input type="text" value="" onkeypress="return isNumber(event)" class="inp required"
        ...
        <div class="inp-row" id="barcodeRow" style="justify-content: flex-start;gap: 24px;">
            <div class="inp-group">
                ...
                <div class="barcode-inp-cont">
                    <input type="text" class="inp hide-sm" onkeypress="return isNumber(event)" id="txtbarcode"
                        placeholder="product barcode" data-id="txtbarcode" />
        ...
        submitBtn.addEventListener("click", e => {
            ...
            if (isValid()) {
                if (purchaseInvoiceTable.calculateNetTotal() > 0) {
                    ...
                    xmlhttp.open("POST", `services/uploadPurchase.php`, true);
                    xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
                    xmlhttp.send("data=" + JSON.stringify(purchaseObject));
                } else {
                    showAlert("No Purchase Added", "error");
                }
            }
            else {
