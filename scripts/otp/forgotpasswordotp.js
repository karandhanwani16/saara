  ...
  xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
          ...
          if (parseInt(result) == 0) {
           showAlert("cannot resend OTP","error");
          }else {
              location.reload();
          }
      }
  };
  xhttp.open("GET","services/authentication/forgotemailOtpResend.php?email="+selectedEmail, true);
  xhttp.send();
});
    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if(result.status=="success"){
                window.location = "changepassword.php?email="+email;
            }else{
                showAlert(result.message, result.status);
            }
        }
    };
    xhttp.open("GET","services/authentication/forgotemailOtpValidation.php?otp="+otp+"&email="+email, true);
    xhttp.send();
}
