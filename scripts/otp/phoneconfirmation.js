...
submitBtn.addEventListener("click",e => {
  ...
  if (isOtpInputValid) {
      ...
      validatePhoneOtp(finalOtp,selectedPhone);      
  }
});
...
resendBtn.addEventListener("click",e => {
  ...
  xhttp.open("GET","services/authentication/phoneOtpResend.php?phone="+selectedPhone, true);
  xhttp.send();
});
 
function validatePhoneOtp(otp,phone) {
var xhttp = new XMLHttpRequest();
xhttp.onreadystatechange = function() {
if (this.readyState == 4 && this.status == 200) {
  ...
  if (isOtpValid == 1) {
          window.location = "accountSuccessful.html";
      }else{
...
xhttp.open("GET","services/authentication/phoneOtpValidation.php?otp="+otp+"&phone="+phone, true);
xhttp.send();
}
